...
const numArr = []
  ...
  const stringsToNumbs = (numbers) => {
    ...
    for(let i = 0; i < numbers.length; i ++){
      let indexChange = Number(numbers[i])
      numArr.push(indexChange)

      if(numArr[i] % 2 == 0){
        sumEvens = sumEvens + numArr[i]
      }
    ...
    console.log(`index: ${atxIdx}, value: ${numArr[atxIdx]}`);
  }
 ...
 //using a higher order function, create an array of the unique 'weather_state_name' values of the weather array. Your function should return the following array ['Light Cloud', 'Heavy Cloud', 'Showers']
  ...
  const uniqueWeatherStates = (weatherArr) =>{
    
    let weatherStatesHolder = []
    let weatherStates = []
  
    weatherArr.forEach((weatherName)=>{
      weatherStatesHolder.push(weatherName.weather_state_name)
    })
  
   for(let i = 0; i < weatherStatesHolder.length; i++){
    let weatherSelector = weatherStatesHolder[i]

    if(!weatherStates.includes(weatherSelector)){
      weatherStates.push(weatherSelector)
    }
   }
  ...
  const findIdealTemp = (weatherArr) =>{
    let idealTemp = 0
    
    for(let i = 0; i < weatherArr.length; i++){
      if(weatherArr[i].min_temp == 15.915){
        idealTemp = weatherArr[i].id
      }
    }
...
uniqueWeatherStates(weather)
findIdealTemp(weather)
